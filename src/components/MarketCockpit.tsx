/* ===========================================================================
   ...
   identical markup from identical analytics.
   ========================================================================= */
...
import { VOL_REGIME_META, type VixComplex, type StressGauge } from '@/lib/vol';
...
function SimulationBanner({ build }: { build: 'server' | 'browser' }) {
  return (
    <div className="border-b border-amber/30 bg-amber/[0.07] px-3 py-2 flex items-start gap-2.5">
      ...
      <div className="min-w-0">
        ...
        <p className="text-[11px] text-ink-2 leading-relaxed">
          {build === 'browser'
            ? 'This is the browser edition: the entire engine — indicators, signals, options pricing, backtest — is running locally in this tab, on a deterministic market simulator. The analytics are real. The prices are not, and nothing here is a tradeable signal.'
            : 'No market data provider is reachable from this environment, so every price on this screen is produced by the deterministic simulator. The analytics, signal engine and backtester are running for real against that synthetic series — but these are not market prices, and nothing here is a tradeable signal.'}{' '}
          <span className="text-ink-3">
            {build === 'browser' ? (
              <>
                To attach live feeds, run the terminal locally —{' '}
                <a
                  href="https://github.com/nasiraddy00/vibecode/tree/claude/day-trading-signal-website-zcyeeg"
                  target="_blank"
                  rel="noopener noreferrer"
                  className="text-amber/90 underline underline-offset-2 hover:text-amber"
                >
                  source and setup instructions
                </a>
                . Yahoo, Binance and CoinGecko need no API key at all.
              </>
            ) : (
              <>
